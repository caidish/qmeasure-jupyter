...
export function exportSweepQueue(entries: QueueEntry[]): string {
  ...
  // Header (assumes imports are already done in notebook)
  sections.push(`# Generated Sweep Queue
# Generated by QMeasure Jupyter Extension
# Total sweeps: ${entries.length}
# Note: Assumes measureit classes and station are already imported
...
`);
  ...
  entries.forEach((entry, index) => {
    ...
    // Replace the default sweep variable name with our generated one
    // This assumes the setup code uses standard names like s_1D, s_2D, etc.
    let setupCode = entry.code.setup;

    // Try to detect and replace the sweep variable name
    const sweepVarMatch = setupCode.match(/^(\w+)\s*=/m);
    if (sweepVarMatch && sweepVarMatch[1]) {
      const originalVar = sweepVarMatch[1];
      // Replace all occurrences of the original variable with our generated one
