 ...
 */
export function renderSweepCode(code: SweepCode, includeStart: boolean = true): string {
  if (includeStart) {
    return `${code.setup}\n\n${code.start}`;
  }
  return code.setup;
...
export function generateSweep0D(params: Sweep0DParameters): SweepCode {
  ...
  const followParamsCode = params.follow_params.length > 0
    ? params.follow_params.map(p => `${sweepName}.follow_param(${p})`).join('\n')
    : '# No follow parameters specified';

  // Build custom parameters as constructor kwargs
  const customParamsLines = params.custom_params && params.custom_params.length > 0
...
export function generateSimulSweep(params: SimulSweepParameters): SweepCode {
  ...
  const paramEntries = params.params.map(p => {
  ...
  if (params.bidirectional) sweepArgs.push(`    "bidirectional": ${toPython(params.bidirectional)}`);
  if (params.continual) sweepArgs.push(`    "continual": ${toPython(params.continual)}`);
  if (params.save_data !== undefined) sweepArgs.push(`    "save_data": ${toPython(params.save_data)}`);
  if (params.plot_data !== undefined) sweepArgs.push(`    "plot_data": ${toPython(params.plot_data)}`);
  if (params.inter_delay !== undefined) sweepArgs.push(`    "inter_delay": ${params.inter_delay}`);
  if (params.plot_bin !== undefined) sweepArgs.push(`    "plot_bin": ${params.plot_bin}`);
  if (params.err !== undefined) sweepArgs.push(`    "err": ${params.err}`);
  if (params.suppress_output) sweepArgs.push(`    "suppress_output": ${toPython(params.suppress_output)}`);
  ...
  const sweepArgsDict = sweepArgs.length > 0
    ? `sweep_args = {\n${sweepArgs.join(',\n')}\n}`
    : '';
